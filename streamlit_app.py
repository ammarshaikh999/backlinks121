...
.stTextInput > div > div > input:focus { border-color: #c8f135 !important; box-shadow: 0 0 0 1px #c8f135 !important; }
...
def _load_pipeline():
    try:
        ...
        from src.main import evaluate_page, merge_unique_lists, choose_final_action, choose_final_score, choose_final_label
        return True, None
...
def run_row(row, brand_name, niche, allowed_types, timeout, user_agent, config):
    ...
    competitor_domains = split_competitors(str(row.get("competitors", "")).strip())

    page_result = evaluate_page(
        url=url, brand=brand, row_niche=row_niche, notes=notes,
        competitor_domains=competitor_domains, allowed_types=allowed_types,
    ...
    domain_result = evaluate_page(
        url=root_url, brand=brand, row_niche=row_niche, notes=notes,
        competitor_domains=competitor_domains, allowed_types=allowed_types,
        timeout=timeout, user_agent=user_agent, config=config,
    )
    ...
    return {
        ...
        "competitor_domains_linked": " | ".join(merge_unique_lists(
            page_result["competitor_domains_linked"], domain_result["competitor_domains_linked"])),
        "competitor_domains_mentioned": " | ".join(merge_unique_lists(
            page_result["competitor_domains_mentioned"], domain_result["competitor_domains_mentioned"])),
        "competitor_anchor_examples": " | ".join(merge_unique_lists(
            page_result["competitor_anchor_examples"], domain_result["competitor_anchor_examples"])[:10]),
...
with st.sidebar:
    ...
    all_types = [
        "guest_post", "profile", "social_bookmarking", "web_directory",
        "article", "business_listing", "email_submission", "directory_business",
    ]
    allowed_types = st.multiselect("types", options=all_types, default=all_types, label_visibility="collapsed")
...
ok, err = _load_pipeline()
if not ok:
    st.error(f"❌ Could not load pipeline modules.\n\n`{err}`\n\nMake sure you're running from the project root directory.")
    st.stop()
...
if uploaded:
    ...
    if run_btn:
        ...
        for i, (_, row) in enumerate(df_input.iterrows()):
            ...
            try:
                rows_out.append(run_row(row, brand_name, niche, allowed_types, timeout, user_agent, config))
            except Exception as e:
...
if "results_df" in st.session_state:
    ...
    for col, val, lbl in [
        (c1, total, "TOTAL"), (c2, qualified, "QUALIFIED"),
    ...
    def show_df(subset):
        ...
        st.dataframe(
            subset[cols].style.applymap(score_color, subset=["priority_score"] if "priority_score" in cols else []),
